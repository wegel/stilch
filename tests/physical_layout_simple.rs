...
"#;
    ...
    env.start_compositor(&[
        "--test",
        "--logical-size",
        "1920x1080", // First display
        "--ascii-output",
        "1920x1080+0+1080", // Second display below
        "--config",
        &config_path,
    ])?;
    ...
    if final_pos.1 >= 1080.0 {
        ...
        println!(
            "❌ FAILED: Cursor stuck at y={}, should be >= 1080",
            final_pos.1
        );
        return Err(format!(
            "Cursor didn't jump to second display, stuck at y={}",
            final_pos.1
        )
        .into());
    }
...
fn get_cursor_position(client: &TestClient) -> Result<(f64, f64), Box<dyn std::error::Error>> {
    ...
    if let Some(message) = response["message"].as_str() {
        ...
        let x = data["data"]["x"].as_f64().ok_or("No cursor X position")?;
        let y = data["data"]["y"].as_f64().ok_or("No cursor Y position")?;
