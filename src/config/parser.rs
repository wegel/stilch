    ...
    while i < parts.len() {
        match parts[i] {
            ...
            "physical_size" if i + 1 < parts.len() => {
                ...
                if size_parts.len() != 2 {
                    return Err(format!(
                        "Invalid physical_size format: {}. Expected WIDTHxHEIGHT[mm|in]",
                        size_str
                    )
                    .into());
                }

                let width: f64 = size_parts[0]
                    .parse()
                    .map_err(|_| format!("Invalid width in physical_size: {}", size_parts[0]))?;
                let height: f64 = size_parts[1]
                    .parse()
            ...
            "physical_position" if i + 1 < parts.len() => {
                ...
                if pos_parts.len() != 2 {
                    return Err(format!(
                        "Invalid physical_position format: {}. Expected X,Y[mm|in]",
                        pos_str
                    )
                    .into());
                }

                let x: f64 = pos_parts[0]
                    .parse()
                    .map_err(|_| format!("Invalid x in physical_position: {}", pos_parts[0]))?;
                let y: f64 = pos_parts[1]
                    .parse()
