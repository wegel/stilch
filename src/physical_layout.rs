...
impl PhysicalDisplay {
    ...
    pub fn physical_to_logical(
        &self,
        physical_mm: Point<f64, Physical>,
    ) -> Option<Point<f64, Logical>> {
        let bounds = self.physical_bounds();

        // Check if point is within this display's physical bounds (with small tolerance)
        if physical_mm.x < bounds.loc.x - 0.5
            || physical_mm.x > bounds.loc.x + bounds.size.w + 0.5
            || physical_mm.y < bounds.loc.y - 0.5
            || physical_mm.y > bounds.loc.y + bounds.size.h + 0.5
        {
            return None;
        }
    ...
    pub fn logical_to_physical(&self, logical: Point<f64, Logical>) -> Point<f64, Physical> {
        ...
        let bounds = self.physical_bounds();
        Point::from((mm_x + bounds.loc.x, mm_y + bounds.loc.y))
    }
}
...
impl PhysicalLayoutManager {
    ...
    pub fn add_display(&mut self, display_info: PhysicalDisplay) {
        info!(
            ...
            display_info.name,
            display_info.physical_size_mm.w,
            display_info.physical_size_mm.h,
            display_info.physical_position_mm.x,
            display_info.physical_position_mm.y,
            display_info.scale
        );

        self.displays
            .insert(display_info.name.clone(), display_info);
    }
    ...
    pub fn display_at_physical_point(
        &self,
        point: Point<f64, Physical>,
    ) -> Option<&PhysicalDisplay> {
        self.displays
            .values()
            .find(|d| d.physical_bounds().contains(point.to_i32_round()))
    }

    /// Find the nearest display to a physical point (for gap jumping)
    pub fn nearest_display_to_point(
        &self,
        point: Point<f64, Physical>,
    ) -> Option<&PhysicalDisplay> {
        self.displays.values().min_by_key(|d| {
    ...
    pub fn handle_relative_motion(
        ...
        let display_bounds =
            Rectangle::new(current_disp.logical_position, current_disp.logical_size);
        ...
        let physical_delta =
            Point::<f64, Physical>::from((delta.x * mm_per_logical_x, delta.y * mm_per_logical_y));
        ...
        let potential_targets = if crossed_bottom {
            ...
            self.displays
                ...
                .filter(|d| {
                    let bounds = d.physical_bounds();
                    let is_below = bounds.loc.y
                        > current_disp.physical_position_mm.y + current_disp.physical_size_mm.h
                            - 10.0;
        ...
        if let Some(gap_target) = potential_targets {
            ...
            let jumped_physical = Point::from((
                target_physical
                    .x
                    .clamp(bounds.loc.x + 0.1, bounds.loc.x + bounds.size.w - 0.1),
                target_physical
                    .y
                    .clamp(bounds.loc.y + 0.1, bounds.loc.y + bounds.size.h - 0.1),
            ));
            ...
            if let Some(new_pos) = gap_target.physical_to_logical(jumped_physical) {
                ...
                warn!(
                    "Failed to convert jumped physical position to logical for display '{}'",
                    gap_target.name
                );
            }
        }
...
mod tests {
    ...
    fn test_physical_cursor_continuity() {
        ...
        assert!(
            new_pos.x < 1707.0 + 1920.0,
            "Cursor should still be within Display2"
        );
    ...
    fn test_gap_jumping() {
        ...
        assert!(
            new_pos.x >= 2000.0 && new_pos.x < 2100.0,
            "Cursor should have jumped to Display2's left edge, got x={}",
            new_pos.x
        );
        ...
        assert!(
            new_pos2.x < 1920.0 && new_pos2.x > 1800.0,
            "Cursor should have jumped to Display1's right edge, got x={}",
            new_pos2.x
        );
    }
}
