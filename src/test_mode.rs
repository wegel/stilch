...
pub fn run_test_mode(config: TestModeConfig) {
    ...
    let mut physical_displays = Vec::new();
    {
        ...
        info!("Checking {} outputs for physical layout configuration", outputs.len());
        for output in outputs {
            let output_name = output.name();
            info!("Checking output '{}' for physical layout config", output_name);
            ...
            if let Some(output_config) = state.config.outputs.iter().find(|o| o.name == output_name) {
                ...
                if let (Some(physical_size_mm), Some(physical_position_mm)) = 
                    (output_config.physical_size_mm, output_config.physical_position_mm) 
                {
                    // Get output properties
                    let current_mode = output.current_mode().unwrap_or_else(|| {
                        smithay::output::Mode {
                            size: (1920, 1080).into(),
                            refresh: 60_000,
                        }
                    ...
                    let position = state.space().output_geometry(&output)
                        .map(|g| g.loc)
