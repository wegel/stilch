...
use crate::{focus::PointerFocusTarget, state::Backend, StilchState};
...
impl WindowElement {
    ...
    pub fn title(&self) -> String {
        // Extract title from the XDG toplevel surface data
        if let Some(toplevel) = self.0.toplevel() {
            smithay::wayland::compositor::with_states(toplevel.wl_surface(), |states| {
                states
                    ...
                    .and_then(|data| {
                        let role = data.lock().ok()?;
                        role.title.clone()
                    })
                    .unwrap_or_else(|| {
                        // Fallback to window ID if title not set
                        if let Some(id) = self.user_data().get::<usize>() {
                            format!("Window {id}")
                        } else {
                            "Window".to_string()
                        }
    ...
    pub fn app_id(&self) -> Option<String> {
        // Extract app_id from the XDG toplevel surface data
        if let Some(toplevel) = self.0.toplevel() {
            smithay::wayland::compositor::with_states(toplevel.wl_surface(), |states| {
                states
                    .data_map
...
impl<BackendData: Backend> PointerTarget<StilchState<BackendData>> for SSD {
    fn enter(
        &self,
    ...
    fn frame(&self, _seat: &Seat<StilchState<BackendData>>, _data: &mut StilchState<BackendData>) {}
    fn leave(
        &self,
        _seat: &Seat<StilchState<BackendData>>,
        _data: &mut StilchState<BackendData>,
...
impl<BackendData: Backend> TouchTarget<StilchState<BackendData>> for SSD {
    fn down(
        &self,
        seat: &Seat<StilchState<BackendData>>,
        data: &mut StilchState<BackendData>,
