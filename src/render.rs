...
smithay::backend::renderer::element::render_elements! {
    ...
    TabBar=smithay::backend::renderer::element::memory::MemoryRenderBufferRenderElement<R>,
    #[cfg(feature = "debug")]
...
impl<R: Renderer> std::fmt::Debug for CustomRenderElements<R> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::TabBar(arg0) => f.debug_tuple("TabBar").field(arg0).finish(),
            #[cfg(feature = "debug")]
...
pub fn collect_tab_bar_data<B>(state: &StilchState<B>, output: &Output) -> Vec<TabBarData>
where
    B: Backend,
{
    ...
    let vo_id = vo_ids[0]; // Use first virtual output for this physical output

    let Some(vo) = state.virtual_output_manager.get(vo_id) else {
        return tab_bars;
    };

    if let Some(workspace_num) = vo.active_workspace() {
        let workspace_id = crate::workspace::WorkspaceId::new(workspace_num as u8);
        if let Some(workspace) = state.workspace_manager.get_workspace(workspace_id) {
            // Check if there's a fullscreen window - if so, don't show tab bars
            if let Some(fullscreen_id) = workspace.fullscreen_window {
                // Check what type of fullscreen it is
                if let Some(managed_window) = state.window_registry().get(fullscreen_id) {
                    if let crate::window::WindowLayout::Fullscreen { mode, .. } =
                        &managed_window.layout
                    {
                        match mode {
                            // Container fullscreen shows tab bar (window fullscreens within its container)
                            crate::window::FullscreenMode::Container => {
                                // Continue to show tab bar for container fullscreen
                            }
                            // Virtual and Physical fullscreen hide tab bars
                            crate::window::FullscreenMode::VirtualOutput
                            | crate::window::FullscreenMode::PhysicalOutput => {
                                // Don't show any tab bars
                                return tab_bars;
                            }
                        }
                    }
            ...
            let tabbed_containers = layout.find_tabbed_containers();

            for (container_geometry, tabs) in tabbed_containers {
...
/// Generate tab bar elements from collected data
pub fn generate_tab_bar_elements<R>(
    renderer: &mut R,
    tab_bar_data: &[TabBarData],
    scale: Scale<f64>,
