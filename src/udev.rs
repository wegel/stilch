    ...
    event_loop
        ...
        .insert_source(libinput_backend, move |mut event, _, data| {
            ...
            if let InputEvent::DeviceAdded { device } = &mut event {
                // Apply input device configuration from config
                data.apply_input_config(device);
...
impl StilchState<UdevData> {
                    ...
                    (config.physical_size_mm, config.physical_position_mm)
                {
                    ...
                    if self.physical_layout.is_none() {
                        self.physical_layout =
                            Some(crate::physical_layout::PhysicalLayoutManager::new());
                    }
    ...
    fn device_changed(&mut self, node: DrmNode) {
        ...
        crate::shell::fixup_positions_with_config(
            self.space_mut(),
            pointer_location,
            &output_configs,
        );
